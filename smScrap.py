...
class AllAdInfo:
    ...
    def __init__(self, keyword, location=None, page=1):
        ...
        self.page = page
        self.complete_url = {'search_category': 'all', 'search_location': self.location, 'q': self.keyword,
                             'page': self.page}
...
def main(user_input_search_entry, user_input_location_entry, user_input_num_pages_entry, user_input_filename_entry):
    ...
    def get_ad_details(all_urls):
        ...
        for singleUrl in all_urls:
            ...
            print(str(counts_of_completed ) + '/' + str(ad_links_count) + '({}%) Completed'.format(
                percent_completed))
            ...
            try:
                ...
                if phone_number_for_single_ad.startswith("07") or phone_number_for_single_ad.startswith("***"):
    ...
    for title, name, phone_number, link in zip(title_ad_for_all_pages, name_for_all_pages, phone_number_for_all_pages,
                                                 ad_links_to_be_kept):
        file.write (
            title.replace(',', '|') + ',' + name.replace(',', '|') + ',' + phone_number + ',' + link + '\n')
